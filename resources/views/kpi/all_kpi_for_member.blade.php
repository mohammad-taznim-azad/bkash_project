@extends('layouts.main')
@section('title'){{ 'All Assigned Kpi' }}@endsection
@section('header.css')
    <style>

    </style>
@endsection
@section('main.content')
    <div class="page-body">
        <div class="container-fluid">
            <div class="page-title">
                <div class="row">
                    <div class="col-6">
                        <h3>All Assigned Kpi</h3>
                    </div>
                    <div class="col-6">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('index') }}"><i class="fa fa-home"></i></a></li>
                            <li class="breadcrumb-item">Settings</li>
                            <li class="breadcrumb-item active">All Assigned Kpi</li>
                        </ol>
                    </div>
                </div>
            </div>
        </div>
        <!-- Container-fluid starts-->
        <div class="container-fluid">
            <div class="row">
                <!-- Zero Configuration  Starts-->
                <div class="col-sm-12">
                    <div class="card">
                        <div class="card-body">
                            {{-- <div class="text-end mb-3">
                                <a href="{{ route('kpi.create') }}" class="btn btn-md btn-info " ><i class="fa fa-plus"></i>Create New</a>
                            </div> --}}
                            <div class="table-responsive">
                                <table id="kpiTable" class="table table-striped"></table>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- Zero Configuration  Ends-->
            </div>
        </div>
        <!-- Container-fluid Ends-->
    </div>
@endsection
@section('footer.js')
    <script>
        $(document).ready(function () {
            $('#kpiTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    "url": "{{route('kpi.all_assigned_kpi_list')}}",
                    "type": "POST",
                    data: function (d) {
                        d._token = "{{ csrf_token() }}";
                    },
                },
                columns: [
                    {title: 'ID', data: 'id', name: 'id', className: "text-center", orderable: true, searchable: true},
                    {title: 'Type Name', data: 'type', name: 'type', className: "text-center", orderable: true, searchable: true},   
                    {title: 'SubType Name', data: 'subtype', name: 'subtype', className: "text-center", orderable: true, searchable: true},  
                    {title: 'Topics Name', data: 'topic', name: 'topic', className: "text-center", orderable: true, searchable: true},  
                    {title: 'Assigned By', data: 'added_by', name: 'added_by', className: "text-center", orderable: true, searchable: true}, 
                    {title: 'Assigned To', data: 'member', name: 'member', className: "text-center", orderable: true, searchable: true}, 
                    {title: 'Date', data: 'date', name: 'date', className: "text-center", orderable: true, searchable: true},
                    {title: 'Status', data: 'status', name: 'status', className: "text-center", orderable: true, searchable: true},
                    {title: 'Assigned Files', data: 'Downloads', name: 'Downloads', className: "text-center", orderable: true, searchable: true},                     
                    {title: 'Feedback Files', data: 'FeedBackFile', name: 'FeedBackFile', className: "text-center", orderable: true, searchable: true},  
                    {title: 'Target', data: 'target', name: 'target', className: "text-center", orderable: true, searchable: true},  
                    {title: 'Total Complete Files', data: 'total_complete', name: 'total_complete', className: "text-center", orderable: true, searchable: true},     
                  
                    { 
                    title: 'Action', 
                    className: "text-center", 
                    data: function (data) {
                        let buttons = '';                       
                      
                        if (data.permissions.includes('kpi.kpi_feedback')) {
                            buttons += '<a title="feedback" class="btn btn-primary btn-sm" data-panel-id="' + data.id + '" onclick="kpiFeedback(this)"><i class="fa fa-eye"></i></a>';
                        }
                        if (data.permissions.includes('kpi.complete_kpi')) {
                            buttons += ' <a title="finished kpi" class="btn btn-warning btn-sm" data-panel-id="' + data.id + '" onclick="finishedKpi(this)"><i class="fa fa-book"></i></a>';
                        }
                        return buttons || 'No Actions Available'; 
                    }, 
                    orderable: false, 
                    searchable: false
                }
                ],              
                
            });
        });

        function kpiFeedback(x) {
            let btn = $(x).data('panel-id');
            let url = '{{route("kpi.kpi_feedback", ":id") }}';
            window.location.href = url.replace(':id', btn);
        }

        function finishedKpi(x) {
            let btn = $(x).data('panel-id');
            let url = '{{route("kpi.complete_kpi", ":id") }}';
            window.location.href = url.replace(':id', btn);
        }

        function deleteKpi(x) {
            let id = $(x).data('panel-id');
            if (!confirm("Delete This Sub Type")) {
                return false;
            }
            $.ajax({
                type: 'POST',
                url: "{{ route('kpi_subtype.delete') }}",
                cache: false,
                data: {
                    _token: "{{ csrf_token() }}",
                    'id': id
                },
                success: function (data) {
                    toastr.success('Sub Type deleted successfully!');
                    $('#subtypeTable').DataTable().clear().draw();
                },
            });
        }
    </script>
@endsection
